        ...
        if num+1 == num_cnt-1:
            ...
            if f_s*f_e < 0:
                ...
                cx, cy = pointsCrossLine(
                    start_point[0][0], start_point[0][1], end_point[0][0], end_point[0][1], a, b, c)
                new_cnt.append(np.array([[cx, cy]], np.int0))
                new_cnt = np.array(new_cnt, np.int0)
                cnts.append(new_cnt)
...
def find_wall_lines(img, img_pre,
    ...
    for line in lines_fastLSD:
        ...
        x0, x1, x2, x3, x4, x5, y0, y1, y2, y3, y4, y5 = int(x0), int(x1), int(x2), int(
            x3), int(x4), int(x5), int(y0), int(y1), int(y2), int(y3), int(y4), int(y5)
                ...
                for x_line in range(x_min, x_max+1):
                    if line_flg[y_line, x_line] == 1:
                        ...
                        # 若有交叉，以长度长的为准
                            ...
                            l_befor = (wall_lines[no_befor][0]-wall_lines[no_befor][2])**2+(
                                wall_lines[no_befor][1]-wall_lines[no_befor][3])**2
                            if l_now > l_befor:
                                line_map[y_line, x_line] = no_line
                                K_map[y_line, x_line] = k_wallLine
            # add wall lines
                    ...
                    for x_detect_r in range(-r_detect, r_detect+1):
                        ...
                        if [y_detect, x_detect] in has_detect_position:
                            ...
                            if line_map[y_detect, x_detect] > 0 and is_detected is False:
                                ...
                                this_len = (
                                    (this_wall_line[0]-this_wall_line[2])**2+(this_wall_line[1]-this_wall_line[3])**2)**0.5
                                # 赋值新的预测像素所属wall line的长度
                                len_dis_k_line_map[y_point, x_point] = this_len
                                # 赋值新的预测像素所属wall line的序号
                                new_line_map[y_point,
                                             x_point] = line_map[y_detect, x_detect]
                                # 赋值斜率特征图
                                K_map[y_point, x_point] = k_lines[line_map[y_detect, x_detect]-1]
                                is_detected = True
                                return
                else:
                        ...
                        for ddx in range(1, l_d+1):
                            ...
                            if line_map[y_line, x_k] > 0:
                                ...
                                if is_K_same(k_line, k_detect):
                                    ...
                                    direct_x1, direct_y1, direct_x_d, direct_y_d = direct_detect
                                    vector_wall_i_detect = [
                                        direct_x_d-direct_x1, direct_y_d-direct_y1]  # wall line 的方向向量
                                    # 探测点方向是否与原wall line方向一致
                                    is_same_direct = True if (
                                        vector_wall_i_detect[0]*vector_wall_i[0]+vector_wall_i_detect[1]*vector_wall_i[1]) >= 0 else False
                                    # 同向时
                                    if is_same_direct:
                                ...
                                if img_bool[y_detect, x_detect] == 1 and is_fill_by_same_line[y_detect, x_detect] == 0:
                                    ...
                                    if new_line_map[y_detect, x_detect] == 0 or (new_line_map[y_detect, x_detect] != 0 and dis_k_line_map[y_detect, x_detect] > dis_pix_detect):
                                        ...
                                        new_line_map[y_detect,
                                                     x_detect] = new_no
                                        dis_k_line_map[y_detect,
                                                       x_detect] = dis_pix_detect
            ...
            for x_line in range(x_min, x_max+1):
                for y_line in range(y_min, y_max+1):
                    ...
                    if line_flg[y_line, x_line] == 1:
                        ...
                        cv2.line(detection_line, (x_line, y_line),
                                 (int(x_k), int(y_k)), 1, 1)
                        ...
                        while has_passed[y_k, x_k] == 0 and (is_re_wall_line is False) and (is_coupled_wall_line is False):
                            ...
                            position_coupled_wall_line.append(
                                [start_y, start_x])  # 保存路径轨迹，成对wall line点位
                            # 以此点为中心，探测下一个像素点
                                    ...
                                    if isPointIn(seed_x_near_near, seed_y_near_near):
                                        ...
                                        if new_line_map[seed_y_near_near, seed_x_near_near] > 0:
                                            ...
                                            if is_K_same(k_stand, K_map[seed_y_near_near, seed_x_near_near]):
                                                num_same_k_seed += 1
                            if num_same_k_seed >= int(num_k_seed*thr) and num_k_seed >= int(num_near_seed*thr):
                                is_same_k_flag = True
                            if is_same_k_flag:
        ...
        for x_rg in range(x):
            for y_rg in range(y):
                if new_line_map[y_rg, x_rg] > 0 and is_K_same(K_map[y_rg, x_rg], k_rg):
                    k_RG(x_rg, y_rg, k_stand=k_rg, k_rg_map=rg_map,
