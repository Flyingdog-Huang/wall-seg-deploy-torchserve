...
class WallSegmentation(BaseHandler):
    ...
    def preprocess(self, data):
        ...
        for row in data:
            ...
            # logger.warning('input data type: {}'.format(type(image)))
        ...
        # logger.warning('input data shape: {}'.format(images.shape))
        return images

    
    def inference(self, data):
        # logger.warning('before infer data shape: {}'.format(data.shape))
        out=self.model.forward(data)
        # logger.warning('after infer data shape: {}'.format(out.shape))
        return out


    def postprocess(self, data, inf_data):
        # logger.warning('**************postprocess**************')
        wall_vectors=[]
        n_classes=2
        
        # # test handler
        # wall_vector={}
        # wall_vector['n_classes']=n_classes
        ...
        # logger.warning('source data shape: {}'.format(image.shape))
        
        
        ## process pridiction data
        probs = F.softmax(inf_data, dim=1)[0]
        ...
        # logger.warning('predict data shape: {}'.format(img_pre.shape))
        
        ## verctorizaton extraction
        img=image.copy()
    ...
    def handle(self, data, context):
        ...
        return self.postprocess(data, inf_data)